...
__global__ void warp_affine_billinear_kernel(
    uint8_t *src, int src_width, int src_height,
    uint8_t *dst, int dst_width, int dst_height,
){
    /*
    gpu上有很多计算单元core 每个core相当于thread 一个thread处在block的某个地方
    下面进行 局部idx 到 全局idx 的转换
    用一个 thread 负责一个三通道的像素
    */
    ...
    // 从dst反映射回src 根据映射float左边的四个邻居加权平均
    affine_project(matrix.d2i, dx, dy, &src_x, &src_y);
    /*
    已知src_x和src_y 做双线性插值
    */
    if(src_x < -1 || src_x >= src_width || src_y < -1 || src_y >= src_height){
        // out of range
        // src_x < -1时，其高位high_x < 0，超出范围
        // src_x >= -1时，其高位high_x >= 0，存在取值(根据双线性插值的定义 可以偏出一个像素)
    }else{
        int y_low = floorf(src_y);  // 取整数
        int x_low = floorf(src_x);
        int y_high = y_low + 1;
        int x_high = x_low + 1;

        // 对于反映射到src区域外的数值 我们赋值为一个常数
        ...
        // src_line_size 每行 block的数量 3是每个block中通道的数量
        if (y_low >= 0) {
            if (x_low >= 0)
...
void warp_affine_bilinear(
    uint8_t *src, int src_width, int src_height,  // pixels uint8 0~255
    uint8_t *dst, int dst_width, int dst_height,
){
    // 需要多少线程
